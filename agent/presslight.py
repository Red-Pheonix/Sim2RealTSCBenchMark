...
class PressLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        self.ob_generator = LaneVehicleGenerator(
            world, inter_obj, ["lane_count"], average=None
        )
        self.phase_generator = IntersectionPhaseGenerator(
            world, inter_obj, ["phase"], targets=["cur_phase"], negative=False
        )
        self.reward_generator = LaneVehicleGenerator(
            world, inter_obj, ["pressure"], average="all", negative=True
        )
        self.action_space = gym.spaces.Discrete(len(inter_obj.phases))
        if self.phase:
            if self.one_hot:
                self.ob_length = self.ob_generator.ob_length + len(
                    inter_obj.phases
                )  # 32
            else:
        ...
        self.epsilon_decay = Registry.mapping["model_mapping"]["setting"].param[
            "epsilon_decay"
        ]
        self.epsilon_min = Registry.mapping["model_mapping"]["setting"].param[
            "epsilon_min"
        ]
        self.learning_rate = Registry.mapping["model_mapping"]["setting"].param[
            "learning_rate"
        ]
        self.batch_size = Registry.mapping["model_mapping"]["setting"].param[
            "batch_size"
        ]
    ...
    def reset(self):
        ...
        inter_obj = self.world.id2intersection[inter_id]
        self.ob_generator = LaneVehicleGenerator(
            self.world, inter_obj, ["lane_count"], average=None
        )
        self.phase_generator = IntersectionPhaseGenerator(
            self.world, inter_obj, ["phase"], targets=["cur_phase"], negative=False
        )
        self.reward_generator = LaneVehicleGenerator(
            self.world, inter_obj, ["pressure"], average="all", negative=True
        )
    ...
    def _batchwise(self, samples):
        ...
        if self.phase:
            if self.one_hot:
                phase_t = np.concatenate(
                    [
                        utils.idx2onehot(item[1][1], self.action_space.n)
                        for item in samples
                    ]
                )
                phase_tp = np.concatenate(
                    [
                        utils.idx2onehot(item[1][5], self.action_space.n)
                        for item in samples
                    ]
                )
            else:
        ...
        rewards = torch.tensor(
            np.array([item[1][3] for item in samples]), dtype=torch.float32
        )
        # actions:(64,1)
        actions = torch.tensor(
            np.array([item[1][2] for item in samples]), dtype=torch.long
        )
...
class DQNNet(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, dic_agent_conf):
        ...
        self.dense_2 = nn.Linear(
            self.dic_agent_conf.param["d_dense"], self.dic_agent_conf.param["d_dense"]
        )
